...
public class FireExtinguish : MonoBehaviour
{
    ...
    private Dictionary<GameObject, FireOld> fires;
    ...
    private void Start()
    {
        fires = new Dictionary<GameObject, FireOld>();
    }
    ...
    void ShootRaycast()
    {
        if(Physics.Raycast(transform.position, transform.forward, out RaycastHit hitInfo, raycastLength))
        {
            if (hitInfo.collider.CompareTag("FireTrigger"))
            {
                ...
                {
                    fires.Add(currentFire, currentFire.transform.parent.GetComponentInChildren<FireOld>());
